...
class get_covariates:
    def __init__(self,tile_overide=['none']):
        self.tile_overide = tile_overide[0]
        if tile_overide[0] != 'none':
            self.tiles = tile_overide
            self.tiles_idx = self.tiles - 1
    ...
    def get_gldas_info(self,start,end,fname,save_name,save_name_norm):
        ...
        # if this is extrapolation pixels, we still need to
        # normalize by the mean and std from the optimization
        # pixels in order for the coefficients to be valid.
        # Therefore, let's get those values.
        norm_var_array = np.copy(var_array)
        if self.tile_overide != 'none':
            optimization_data = xr.open_dataset(
                '/discover/nobackup/trobinet/from_aws/pso/' +
                'step_2x_env_covariates/outputs/' +
                'gldas_avg_precip.nc4'
            )
            precip_data = optimization_data['vals'].values
            var_mean = np.mean(precip_data)
            var_std = np.std(precip_data)
        else:
    ...
    def get_canopy_info(self,fname,save_name,save_name_norm):
        ...
        if self.tile_overide != 'none':
            ...
            h_data = optimization_data['vals'].values
            var_mean = np.mean(h_data)
            var_std = np.std(h_data)
        else:
            var_mean = np.mean(norm_var_array)
            var_std = np.std(norm_var_array)
        norm_var_array = norm_var_array - var_mean
